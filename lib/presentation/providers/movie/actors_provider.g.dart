...
class ActorsFamily extends Family<AsyncValue<List<Actor>>> {
  ...
  /// See also [actors].
  ActorsProvider call({required int movieId}) {
    return ActorsProvider(movieId: movieId);
  }
...
class ActorsProvider extends AutoDisposeFutureProvider<List<Actor>> {
  /// See also [actors].
  ActorsProvider({required int movieId})
    : this._internal(
        (ref) => actors(ref as ActorsRef, movieId: movieId),
        from: actorsProvider,
